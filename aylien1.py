...
def main():
    ...
    for story in stories:
        ...
        # Extract and display additional details from each article
        categories = story.get('categories', [])
        for category in categories:
            st.write('Category Taxonomy:', category.get('taxonomy', 'N/A'))
            st.write('Category Confident:', category.get('confident', 'N/A'))
            st.write('Category ID:', category.get('id', 'N/A'))

        entities = story.get('entities', {'title': []}).get('title', [])
        for entity in entities:
            st.write('Entity Title Text:', entity.get('text', 'N/A'))

        sentiment = story.get('sentiment', {}).get('title', {}).get('polarity', 'N/A')
        st.write('Sentiment Title Polarity:', sentiment)
